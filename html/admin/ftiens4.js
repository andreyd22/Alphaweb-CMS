...
// ***************************************************************** 
function Folder(folderDescription, hreference, dop_folder_content) //constructor 
{ 
  //constant data 
  this.desc = folderDescription 
  this.hreference = hreference 
  this.dop_content = dop_folder_content 
  this.id = -1   
  this.navObj = 0  
  this.iconImg = 0  
  this.nodeImg = 0  
  this.isLastNode = 0 
  this.ICONPATH="/admin/admin_img/"
...
function Item(itemDescription, itemLink, target, dop_item_content) // Constructor 
{ 
  // constant data 
  this.desc = itemDescription 
  this.link = itemLink    
  this.dop_content = dop_item_content    
...
{ 
  ...
  docW = docW + "<tr>" + leftSide + "<td valign=top>"
  if (USEICONS)
      docW = docW + "<a  class=m2 " + fullLink  + " id=\"itemIconLink"+this.id+"\">" + "<img id='itemIcon"+this.id+"' " + "src='"+this.iconSrc+"' border=0>" + "</a>"
  else
  ...
  if (USETEXTLINKS) 
    docW = docW + "<a  class=m2 " + fullLink + " id=\"itemTextLink"+this.id+"\">" + this.desc + "</a>"
  else 
    docW = docW + this.desc
...
function highlightObjLink(nodeObj) {
  ...
  lastClicked = nodeObj;
  if (PERSERVESTATE)
    SetCookie('highlightedFolderTreeLink', nodeObj.getID());
}

function gFld(description, hreference, dop_fld_content) 
{ 
  folder = new Folder(description, hreference, dop_fld_content) 
  return folder 
} 
 
function gLnk(optionFlags, description, linkData, dop_doc_content) 
{ 
  var fullLink = "";
  var targetFlag = "";
  var target = "";
  ...
  linkItem = new Item(description, protocol+linkData, target, dop_doc_content)
  return linkItem 
} 
...
function CookieBranding(name) {
  if (typeof foldersTree.treeID != "undefined")
    return name+foldersTree.treeID //needed for multi-FolderTree sites. make sure treeId does not contain cookieCutter
  else
    return name
}
...
{ 
  ...
  if (PERSERVESTATE && GetCookie('highlightedFolderTreeLink')!=null  && GetCookie('highlightedFolderTreeLink')!="") {
