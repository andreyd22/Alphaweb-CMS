...
sub list_news {
 ...
 my $col_records=$ref->{user_db}->{data}->{$ref->{id}}->{params}->{col_records}||50;
 my $dbh=dbconnect;
 ...
 my $col="select count(*) from partners where idr='$ref->{id}' and visible='1'";
 my $count=$dbh->selectrow_array($col);
 my $kol;
 if($count%$CountPage==0){$kol=int($count/$CountPage);}else{$kol=int($count/$CountPage)+1;}
 #переход по страницам
 my $sel="select * from partners where idr='$ref->{id}' and visible='1' order by sort desc, id desc limit $off,$CountPage";
...
#         my $data_print="$day/$month/$year $hour:$min";
    ...
    if($ref->{slovo}){
	    ...
	    $url1="/partners/$ref->{id}.html?id_r=$ref->{id_r}&slovo=$ref->{slovo}";
    }
...
sub full_news {
 ...
 #Выводим новость
 my $dbh=dbconnect;
 my $sel="select * from partners where id='$ref->{data_id}' and idr='$ref->{id}'";
...
#    my $data_print="$day/$month/$year $hour:$min";
    ...
    $ref->{ref_data}->{link}=~s/http:\/\///gi;
    $back="/$ref->{user_db}->{data}->{$ref->{id}}->{lang}/$ref->{id}/";
 #Выводим ключевики
 $ref->{title}=$ref->{ref_data}->{name} if $ref->{ref_data}->{name};
